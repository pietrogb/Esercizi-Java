...
Definire una Gui composta da un pannello e tre vottoni etichettati rispettivamente Avvia, Pausa e Stop, la cui pressione soddisfi le sewguenti specifiche.
	...
	- la pressione del tasto Stop deve impostare lo stato di interruzione del thread che fa cambiare il colore al pannello di sfondo. Una volta interrotto, il thread deve terminare la sua esecuzione e non deve essere riattivabile con una successiva pressione del tasto Avvia
	- si ricorda che il metodo public void setBackbround(Color c)  della classe Component imposta a c il colore dello sfondo del componente d'invocazione
...
class ButtonPanel extends JPanel {
	...
	class T extends Thread {
		...
		public void invertiFlag() {runFlag = !runFlag;}
				...
				synchronized(o){
					while(runFlag){
						...
						try { this.sleep(299);
							...
							interrupt();
							System.err.println(isInterrupted() + "Thread interrotto durante lo sleep");
							return;
						}
						/*try{
							o.wait();
							...
							interrupt();
							System.err.println(isInterrupted() + "Thread interrotto durante la wait");
							return;
						}*/
					}
				}
	...
	class Lstart implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			else if (!t.isInterrupted() && !(t.isRunning())) {
				synchronized(o) {
					t.invertiFlag();
					o.notify();
				}
